...
type t = {
  ...
  sklad_2: sklad;
  sklad_3: sklad;
  prehodi : (stanje * char * char * char) list * (stanje * char list * char list * char list) list
...
let dodaj_prehod stanje1 prebrano vrh_s1 vrh_s2 vrh_s3 stanje2 nov_s1 nov_s2 nov_s3 avtomat =
  { avtomat with prehodi = (stanje1, prebrano, vrh_s1, vrh_s2, vrh_s3, stanje2, nov_s1, nov_s2, nov_s3) :: avtomat.prehodi }

let prehodna_funkcija avtomat stanje prebrano vrh_s1 vrh_s2 vrh_s3 =
  match
    List.find_opt
      (fun (stanje1, prebrano', vrh_s1', vrh_s2', vrh_s3', ostalo) -> stanje1 = stanje && prebrano = prebrano'
      && vrh_s1 = vrh_s1' && vrh_s2 = vrh_s2' && vrh_s3 = vrh_s3')
      avtomat.prehodi
  with
  | None -> None
